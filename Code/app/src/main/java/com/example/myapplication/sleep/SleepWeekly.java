...
public class SleepWeekly extends Fragment implements View.OnClickListener{
    private View view;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(SleepFileManager.files.size() >= 7){
            ...
            PlotChart.barChart(view.getContext(), "sleep", barChart, weeklyHrAverage, xLabel);
        }
    ...
    private void startRecyclerView() {
        WeeklyAdapter myAdapter = new WeeklyAdapter(view.getContext(),"sleep");
    ...
    private void getWeeklyAverageData() {
        ...
        if(SleepFileManager.files.size() < 28){
            for(int i = 0; i < daysForAverage; i++) {
                hrAverage.add((double)SleepFileManager.files.get(i).getTotalHoursSlept());
                minAverage.add((double)SleepFileManager.files.get(i).getTotalMinuteSlept());
                dateLabel.add(SleepFileManager.files.get(i).getDate().substring(5)); /** only taking month and day excluding year */
            }
        }
        ...
        /** gets implemented when there is more than 28 days of data and stores average value of total hours and minutes slept of recent 4 weeks */
            ...
            for(int i = 0; i < 28; i++) {
                hrAverage.add((double)SleepFileManager.files.get(i).getTotalHoursSlept());
                minAverage.add((double)SleepFileManager.files.get(i).getTotalMinuteSlept());
                dateLabel.add(SleepFileManager.files.get(i).getDate().substring(5));
            }
        }
