...
public class ActiveMore extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(ReadAndSaveMultipleFile.allData.size() >= 7){
            getSevenDaysData();
            PlotChart.barChart(view.getContext(), "active", barChart, sevenDaysData, xLabel);
        }
    ...
    private void getSevenDaysData() {
        ...
        for (int i = 0; i < days; i++) {
            sevenDaysData.add((double) ReadAndSaveMultipleFile.allData.get(i).getHrActive());
            xLabel.add(ReadAndSaveMultipleFile.allData.get(i).getDate().substring(5));
        }
    }

    private void startRecyclerView() {
        MyAdapter myAdapter = new MyAdapter(view.getContext(),"active");
